...
n_m=$(txt_list_part_get "$txt" "\[master\]" "^$")
n_n=$(txt_list_part_get "$txt" "\[node\]" "^$")
#echo "$txt"
echo "$n_m" | sed "s/^ *#.*//g" | sed "/^ *$/d"
echo "$n_n" | sed "s/^ *#.*//g" | sed "/^ *$/d"


n_m=$(txt_list_part_get2 "$txt" "\[master\]" "^$")
n_n=$(txt_list_part_get2 "$txt" "\[node\]" "^$")
#echo "$txt"
echo "$n_m" | sed "s/^ *#.*//g" | sed "/^ *$/d"
echo "$n_n" | sed "s/^ *#.*//g" | sed "/^ *$/d"
...
txt_list_part_del3 "$p" "$txt"
p=$(cat<<EOF
[node]

k8s-node-18=192.168.56.18
EOF
)
txt_list_part_add3 "$p" "$txt"
